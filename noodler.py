...
class TranscribeContext:
    def __init__(self, name, data, sampling_rate):
        self.name = name
    ...
    def play(self):
        self.play_with_callback(lambda: input())
    
    # plays the configured track from the current playback timestamp and then calls the provided callback.
    # playback ends when the callback returns.
    def play_with_callback(self, callback):
        ...
        def pyaudio_callback(in_data, frame_count, time_info, status):
            nonlocal current_frame, start_frame, end_frame
    ...
    def play_for_tempo(self):
        print("Computing tempo. Hit ENTER every beat until the tempo seems accurate.")
        estimated_tempo = None
        def tempo_callback():
            nonlocal estimated_tempo
            count = 0
            start = None
            next = input()
            while next == "":
                ...
                count += 1
                if count > 1:
                    estimated_tempo = self.compute_tempo(count, start, end)
                    print("Estimated tempo: {} bpm".format(estimated_tempo))
                next = input()
        self.play_with_callback(tempo_callback)
        return estimated_tempo

    def compute_tempo(self, count, start, end):
        if count <= 1:
            return None
        seconds_per_beat = (end - start) / (count - 1)
...
while True:
    ...
    TranscribeContext(os.path.basename(path), data, sampling_rate).run()